...
class NewsViewModel(private val state: SavedStateHandle) : ViewModel() {
        private val key = "liked_ids"
        private val _posts = MutableLiveData<List<NewsCard>>(createPosts())
        val posts: LiveData<List<NewsCard>> = _posts

        private val likedIds: MutableSet<String> = state.get<ArrayList<String>>(key)
            ?.toMutableSet()
            ?: mutableSetOf()

        init {
            // apply likedIds to posts
            _posts.value = _posts.value?.map {
                post -> post.copy(isLiked = likedIds.contains(post.id))
            }
        }

        fun toggleLike(postId: String) {
            val cur = _posts.value ?: return
            val nlist = cur.map { post ->
                if (post.id == postId) {
                    val nLiked = !post.isLiked
                    if (nLiked) likedIds.add(postId) else likedIds.remove(postId)
                    post.copy(isLiked = nLiked)
                } else post
            }

            // save the likedIds to SavedStateHandle
            state.set(key, ArrayList(likedIds))
            _posts.value = nlist
        }
        private fun createPosts(): List<NewsCard> {
            return listOf(
